...
void convert () {
	...
	for (margin = 0; !found && margin < (total / 4); margin++) {
		...
		if (numN == 1 || numN == 3) {
			printf("mala cosa %d, pattern %x\n", numN, pattern);
		}
		//printf("margin %d, pattern %x, ", margin, pattern);
		p = decoders;
		while (((simplePattern*)p)->pattern) {
			//printf("patterns %x == %x ? \n",((simplePattern*)p)->pattern, pattern);
			if (((simplePattern*)p)->pattern == pattern) {
				m += margin;
				found++;
				f++;
				//printf("found with margin: %d, pattern %x,", margin, pattern);
				if (((simplePattern*)p)->flag < 0) {
					//printf("Simple multiplier: %d\n", ((simplePattern*)p)->multiplier);
					value1 = ((simplePattern*)p)->multiplier * quarter;
					printf("%d\n", value1 * 120 / total);
					r1 += value1 * 120 / total;
					r2 += value1 * 120 / total;
				} else {
					//printf("complex: [%d, %d], [%d, %d]\n",((complexPattern*)p)->pad1, ((complexPattern*)p)->mult1, ((complexPattern*)p)->pad2, ((complexPattern*)p)->mult2);
					value1 = values[((complexPattern*)p)->pad1] + ((complexPattern*)p)->mult1 * quarter;
					value2 = ((complexPattern*)p)->mult2 * quarter - values[((complexPattern*)p)->pad2];
			...
			//printf("p: %ld, sizes: %ld, %ld, %ld\n",(long)p, sizeof(simplePattern),sizeof(complexPattern),(((simplePattern*)p)->flag < 0 ? sizeof(simplePattern):sizeof(complexPattern)));
			p += (((simplePattern*)p)->flag < 0 ? sizeof(simplePattern):sizeof(complexPattern));
			//printf("p: %ld\n", (long)p);
		}
	}
...
//		values[pad] = (int)((hi - lo) * calibration[pad]);
	...
	if (f) {
		printf("result: over %d, found: %d, v1: %d, v2: %d, v: %d, error: %d, margin: %d\n",count, f * 100 /count, r1 / f, r2 / f, (r1 + r2) /2 / f, e / f, m / f);
	} else {
		puts("none found");
	}
	return 0;
